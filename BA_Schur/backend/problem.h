...
//    typedef std::unordered_map<unsigned long, std::shared_ptr<Vertex>> HashVertex;
            typedef std::map<unsigned long, std::shared_ptr<Vertex>> HashVertex; // 顶点
            typedef std::unordered_map<unsigned long, std::shared_ptr<Edge>> HashEdge; // 边
            typedef std::unordered_multimap<unsigned long, std::shared_ptr<Edge>> HashVertexIdToEdge;

            EIGEN_MAKE_ALIGNED_OPERATOR_NEW; // 使用这个类时，开辟了一个内存，但是没自定义构造函数，所以在new时调用默认构造函数，导致内存位数不对齐，调用此语句，使指针内存指针对齐
