    ...
    namespace backend {
        ...
        bool Problem::AddVertex(std::shared_ptr<Vertex> vertex) {
            if (verticies_.find(vertex->Id()) != verticies_.end()) {
                ...
                // insert插入单个值到%map
                // pair将两个数据合成一个，两个数据可以是同意类型或者不同类型
        ...
        void Problem::AddOrderingSLAM(const std::shared_ptr<myslam::backend::Vertex> &v) {
            // 如果是Pose顶点
            if (IsPoseVertex(v)) {
                v->SetOrderingId(ordering_poses_); // 设置ID
        ...
        bool Problem::IsPoseVertex(const std::shared_ptr<myslam::backend::Vertex> &v) {
            string type = v->TypeInfo();
            return type == string("VertexPose");
        }

        bool Problem::IsLandmarkVertex(const std::shared_ptr<myslam::backend::Vertex> &v) {
            string type = v->TypeInfo();
...
//    CHECK_EQ(CheckOrdering(), true);
        ...
        void Problem::MakeHessian() {
            ...
            for (auto &edge: edges_) {
                ...
                edge.second->ComputeJacobians(); // 计算雅可比矩阵

                auto jacobians = edge.second->Jacobians(); // 边对应的雅可比
                auto verticies = edge.second->Verticies(); // 边对应的顶点，即要优化的变量
                assert(jacobians.size() == verticies.size());
                for (size_t i = 0; i < verticies.size(); ++i) {  // i表示顶点数,变量

                    auto v_i = verticies[i]; // auto可以在声明变量时根据变量初始值的类型自动为此变量选择匹配的类型。
                    if (v_i->IsFixed()) continue;    // 如果点是固定的，则Hessian 里不需要添加它的信息，也就是它的雅克比为 0

                    auto jacobian_i = jacobians[i];
                    ulong index_i = v_i->OrderingId();  // ID索引
                    ulong dim_i = v_i->LocalDimension(); // 顶点维度

                    MatXX JtW = jacobian_i.transpose() * edge.second->Information(); // J_i^T * W
                    ...
                    b.segment(index_i, dim_i).noalias() -= JtW * edge.second->Residual(); // J^T * \Sigma^{-1} * r
                }
            }
            Hessian_ = H;
            b_ = b;
 ...
 */
        ...
        void Problem::TestMarginalize() {
            ...
            H_marg.block(0, idx, reserve_size,
                         reserve_size - idx - dim) = temp_rightCols; // 0行，marg 中间那个变量 列；总变量维度，总-marg

            H_marg.block(0, reserve_size - dim, reserve_size,
                         dim) = temp_cols; // 从0行，总共变量的维度-marg变量的维度 列；总变量维度，marg变量的维度
