...
void Problem::MakeHessian() {
    ...
    for (auto &edge: edges_) {
        ...
        for (size_t i = 0; i < verticies.size(); ++i)  // i表示顶点数,变量
        {
            ...
            MatXX JtW = jacobian_i.transpose() * edge.second->Information(); // J_i^T * r
...
//        delta_x_ = PCGSolver(H, b_, H.rows() * 2);
        ...
        MatXX Hmm = Hessian_.block(??); // 对应landmark
...
void Problem::TestMarginalize() {
    ...
    // 将 row i 移动矩阵最下面
    Eigen::MatrixXd temp_rows = H_marg.block(??);
    Eigen::MatrixXd temp_botRows = H_marg.block(??);
    ...
    Eigen::MatrixXd tempB = Arm * ??;
    Eigen::MatrixXd H_prior = Arr - ?? * ??;
