...
TrustRegion::TrustRegion(double (*_func)(vector<double>), vector<double> (*_derivative)(vector<double>),
                         vector<vector<double> >(*_heissan)(vector<double>)) : func(_func), derivative(_derivative),
                                                                               heissan(_heissan) {
...
vector<double> TrustRegion::searchMin(vector<double> initPoint) {
    ...
    while (true) { //begin the loop
        ...
        if (rho < 1.0 / 4.0)
            radius = radius / 4;
        else if (rho > 3.0 / 4.0 && getLength(pk) == radius)
