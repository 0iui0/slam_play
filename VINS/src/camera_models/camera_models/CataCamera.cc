...
namespace camodocal {
    ...
    CataCamera::Parameters::Parameters()
            : Camera::Parameters(MEI), m_xi(0.0), m_k1(0.0), m_k2(0.0), m_p1(0.0), m_p2(0.0), m_gamma1(0.0),
    ...
    CataCamera::Parameters::Parameters(const std::string &cameraName,
                                       ...
                                       double u0, double v0)
            : Camera::Parameters(MEI, cameraName, w, h), m_xi(xi), m_k1(k1), m_k2(k2), m_p1(p1), m_p2(p2),
              m_gamma1(gamma1), m_gamma2(gamma2), m_u0(u0), m_v0(v0) {
    }
...
/** 
 * \brief Lifts a point from the image plane to its projective ray:将图像平面上的点提升到投影光线
 *
 * \param p image coordinates
 ...
 */
    ...
    CataCamera::distortion(const Eigen::Vector2d &p_u, Eigen::Vector2d &d_u,
        ...
        double dydmx = k1 * 2.0 * p_u(0) * p_u(1) + k2 * 4.0 * rho2_u * p_u(0) * p_u(1) + p1 * 2.0 * p_u(0) +
                       2.0 * p2 * p_u(1);
        double dxdmy = dydmx;
        double dydmy =
