...
namespace camodocal {
    ...
    Chessboard::Chessboard(cv::Size boardSize, cv::Mat &image)
            : mBoardSize(boardSize), mCornersFound(false) {
        if (image.channels() == 1) {
            ...
            for (int dilations = minDilations; dilations <= maxDilations; ++dilations) {
                ...
                if (flags & CV_CALIB_CB_ADAPTIVE_THRESH) {
                    int blockSize = lround(prevSqrSize == 0 ?
                                           std::min(img.cols, img.rows) * (k % 2 == 0 ? 0.2 : 0.1) : prevSqrSize * 2) |
                                    1;

                    // convert to binary
                    cv::adaptiveThreshold(img, thresh_img, 255, CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY, blockSize,
                                          (k / 2) * 5);
                } else {
                    ...
                    for (int j = 0; j < 4; j++) {
                        ...
                        if (quad->neighbors[j]) {
                            ...
                            if (quadNeighbor->labeled) {
                                ...
                                ChessboardCornerPtr &conCornerCW1 = quadNeighbor->corners[
                                        (connectedNeighborCornerId + 1) % 4];
                                ChessboardCornerPtr &conCornerCW2 = quadNeighbor->corners[
                                        (connectedNeighborCornerId + 2) % 4];
                                ChessboardCornerPtr &conCornerCW3 = quadNeighbor->corners[
        ...
        for (float threshLevel = blackLevel; threshLevel < whiteLevel && !result; threshLevel += 20.0f) {
            ...
            for (size_t i = 0; i < quads.size(); ++i) {
                ...
                if (j + 1 > min_quads_count + i) {
                    ...
                    const int whiteCount = lroundf(
                            floorf(patternSize.width / 2.0f) * floorf(patternSize.height / 2.0f));
